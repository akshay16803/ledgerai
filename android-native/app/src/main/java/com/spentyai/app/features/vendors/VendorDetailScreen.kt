...
fun VendorDetailScreen(
    ...
    onNavigateBack: () -> Unit,
    onNavigateToEdit: (Vendor) -> Unit
) {
    val state by viewModel.uiState.collectAsState()

    // Find the potentially updated vendor from the list
    val displayVendor = state.vendors.find { it.id == vendor.id } ?: vendor

    LaunchedEffect(vendor.id) {
        viewModel.loadVendorBills(vendor.id)
    }

    Scaffold(
        topBar = {
            TopAppBar(
                title = { Text(displayVendor.name, style = SpentyType.Headline) },
                navigationIcon = { IconButton(onClick = onNavigateBack) { Icon(Icons.Filled.ArrowBack, contentDescription = "Back") } },
                actions = {
                    TextButton(onClick = { onNavigateToEdit(displayVendor) }) {
        ...
        Column(
            ...
            SectionCard(title = "Vendor Details") {
                InfoRow("Name", displayVendor.name)
                if (!displayVendor.email.isNullOrBlank()) InfoRow("Email", displayVendor.email)
                if (!displayVendor.phone.isNullOrBlank()) InfoRow("Phone", displayVendor.phone)
                if (!displayVendor.taxId.isNullOrBlank()) InfoRow("GSTIN", displayVendor.taxId)
                displayVendor.address?.let { addr ->
                    val addressStr = listOfNotNull(addr.street, addr.city, addr.state, addr.postalCode, addr.country)
                        .filter { it.isNotBlank() }.joinToString(", ")
                    if (addressStr.isNotBlank()) InfoRow("Address", addressStr)
                }
                if (!displayVendor.notes.isNullOrBlank()) InfoRow("Notes", displayVendor.notes)
            }
            ...
            SectionCard(title = "Financial Summary") {
                Row(
                    ...
                    SummaryTile("Billed", displayVendor.totalBilled, SpentyPrimary)
                    SummaryTile("Paid", displayVendor.totalPaid, SpentySuccess)
                    SummaryTile("Outstanding", displayVendor.outstandingBalance, SpentyError)
                }
            }
            ...
            SectionCard(title = "Bills") {
                if (state.isLoadingBills) {
                ...
                } else if (state.vendorBills.isEmpty()) {
                    Text(
                    ...
                    state.vendorBills.forEachIndexed { index, bill ->
                        BillRow(bill)
                        if (index < state.vendorBills.size - 1) {
...
private fun BillRow(bill: Bill) {
    Row(
        ...
        Column {
            Text(
                bill.billNumber?.ifBlank { "Bill" } ?: "Bill",
                style = SpentyType.Subheadline.copy(fontWeight = FontWeight.SemiBold)
            )
            if (bill.issueDate.isNotBlank()) {
                Text(
                    formatDateDisplay(bill.issueDate),
