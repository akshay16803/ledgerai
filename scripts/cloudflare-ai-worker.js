 ...
 *
 * Required vars:
 * - ALLOWED_ORIGIN       (must be set to your LedgerAI web origin)
 *
 * Authentication options (at least one must be configured):
 * - Preferred production mode:
 *   - SUPABASE_URL                    (for JWKS + issuer discovery)
 *   - optional SUPABASE_JWKS_URL      (override JWKS endpoint)
 *   - optional SUPABASE_JWT_ISSUER    (override expected issuer)
 *   - optional SUPABASE_JWT_AUDIENCE  (defaults to "authenticated")
 * - Fallback/admin mode:
 *   - LEDGERAI_SHARED_KEY             (legacy shared-key auth)
 *
...
const JOB_TTL_SECONDS = 14 * 24 * 60 * 60; // 14 days
...
const SUPPORTED_JWT_ALGS = new Set(["RS256", "RS384", "RS512", "ES256", "ES384", "ES512"]);
...
// ============================================================================
// UTILITY FUNCTIONS
// Response helpers, validation, normalization
// ============================================================================
...
function resolveAllowedOrigin(origin = "", allowedOrigin = "") {
  const allowed = normalizeAllowedOrigins(allowedOrigin);
...
function corsHeaders(origin, allowedOrigin = "") {
  const allow = resolveAllowedOrigin(origin, allowedOrigin);
  return {
    ...(allow ? { "Access-Control-Allow-Origin": allow } : {}),
    "Access-Control-Allow-Methods": "GET,POST,OPTIONS",
    "Access-Control-Allow-Headers": "Content-Type,Authorization,x-ledgerai-key",
    Vary: "Origin",
...
function resolveSupabaseJwksUrl(env) {
...
function resolveSupabaseIssuer(env) {
  const explicit = String(env.SUPABASE_JWT_ISSUER || "").trim().replace(/\/+$/, "");
  if (explicit) return explicit;
  const base = normalizeSupabaseUrl(env.SUPABASE_URL || "");
  if (!base) return "";
  return `${base}/auth/v1`;
...
function resolveSupabaseAudience(env) {
  return String(env.SUPABASE_JWT_AUDIENCE || "authenticated").trim() || "authenticated";
}

// ============================================================================
// AUTHENTICATION FUNCTIONS
// JWT verification, shared key auth, request authentication
// ============================================================================

function hasSharedKeyAuth(env) {
  return Boolean(String(env.LEDGERAI_SHARED_KEY || "").trim());
}

function hasSupabaseJwtAuth(env) {
  return Boolean(resolveSupabaseJwksUrl(env) && resolveSupabaseIssuer(env));
}

/**
 * Check for configuration errors in worker environment
 */
function workerConfigError(env) {
  if (!normalizeAllowedOrigins(env.ALLOWED_ORIGIN || "").length) {
    return "ALLOWED_ORIGIN is required";
  }
  if (!hasSharedKeyAuth(env) && !hasSupabaseJwtAuth(env)) {
    return "Configure either SUPABASE_URL (or SUPABASE_JWKS_URL + SUPABASE_JWT_ISSUER) or LEDGERAI_SHARED_KEY";
  }
  return "";
}
...
function decodeJwtPartJson(part = "") {
  const bytes = base64UrlToBytes(part);
  const text = new TextDecoder().decode(bytes);
  return JSON.parse(text || "{}");
}

function jwtImportParams(alg = "") {
  if (alg === "RS256") return { name: "RSASSA-PKCS1-v1_5", hash: { name: "SHA-256" } };
  if (alg === "RS384") return { name: "RSASSA-PKCS1-v1_5", hash: { name: "SHA-384" } };
  if (alg === "RS512") return { name: "RSASSA-PKCS1-v1_5", hash: { name: "SHA-512" } };
...
function jwtVerifyParams(alg = "") {
  if (alg === "RS256") return { name: "RSASSA-PKCS1-v1_5", hash: { name: "SHA-256" } };
  if (alg === "RS384") return { name: "RSASSA-PKCS1-v1_5", hash: { name: "SHA-384" } };
  if (alg === "RS512") return { name: "RSASSA-PKCS1-v1_5", hash: { name: "SHA-512" } };
  if (alg === "ES256") return { name: "ECDSA", hash: { name: "SHA-256" } };
  if (alg === "ES384") return { name: "ECDSA", hash: { name: "SHA-384" } };
  if (alg === "ES512") return { name: "ECDSA", hash: { name: "SHA-512" } };
  return null;
}

async function loadJwks(env, forceRefresh = false) {
  const url = resolveSupabaseJwksUrl(env);
  if (!url) throw new Error("SUPABASE JWKS URL is not configured");
  if (
    !forceRefresh
    && jwksCache.url === url
    && jwksCache.expiresAt > Date.now()
    && jwksCache.keysByKid.size
  ) {
...
async function getCryptoKeyForJwt(env, kid = "", alg = "") {
  const cacheKey = `${kid}:${alg}`;
  const cached = cryptoKeyCache.get(cacheKey);
  if (cached && cached.expiresAt > Date.now()) return cached.key;
  const keys = await loadJwks(env, false);
  let jwk = keys.get(kid);
  if (!jwk) {
    const fresh = await loadJwks(env, true);
    jwk = fresh.get(kid);
  }
  if (!jwk) throw new Error("JWT signing key not found");
  const params = jwtImportParams(alg);
  if (!params) throw new Error(`JWT algorithm ${alg} is not supported`);
  const key = await crypto.subtle.importKey("jwk", jwk, params, false, ["verify"]);
  cryptoKeyCache.set(cacheKey, { key, expiresAt: Date.now() + JWKS_CACHE_TTL_MS });
  return key;
}

function verifyJwtClaims(payload, env) {
  const nowSeconds = Math.floor(Date.now() / 1000);
  const exp = Number(payload?.exp || 0);
  if (!Number.isFinite(exp) || exp <= 0 || nowSeconds > exp + JWT_CLOCK_SKEW_SECONDS) {
    throw new Error("JWT is expired");
  }
  const nbf = Number(payload?.nbf || 0);
  if (Number.isFinite(nbf) && nbf > 0 && nowSeconds + JWT_CLOCK_SKEW_SECONDS < nbf) {
    throw new Error("JWT is not active yet");
  }
  const iss = String(payload?.iss || "");
  const expectedIssuer = resolveSupabaseIssuer(env);
  if (expectedIssuer && iss !== expectedIssuer) {
    throw new Error("JWT issuer mismatch");
  }
  const expectedAudience = resolveSupabaseAudience(env);
  if (expectedAudience) {
    const aud = payload?.aud;
    const matches = Array.isArray(aud)
      ? aud.includes(expectedAudience)
      : String(aud || "") === expectedAudience;
    if (!matches) throw new Error("JWT audience mismatch");
  }
  const sub = String(payload?.sub || "").trim();
  if (!sub) throw new Error("JWT subject is missing");
  return sub;
}

async function verifySupabaseBearerToken(token, env) {
  if (!hasSupabaseJwtAuth(env)) {
    throw new Error("Supabase JWT verification is not configured");
  }
  const parts = String(token || "").split(".");
  if (parts.length !== 3) throw new Error("Malformed JWT");
  const [encodedHeader, encodedPayload, encodedSig] = parts;
  const header = decodeJwtPartJson(encodedHeader);
  const payload = decodeJwtPartJson(encodedPayload);
  const alg = String(header?.alg || "").trim();
  const kid = String(header?.kid || "").trim();
  if (!SUPPORTED_JWT_ALGS.has(alg)) throw new Error(`Unsupported JWT algorithm: ${alg || "unknown"}`);
  if (!kid) throw new Error("JWT missing key id");

  const key = await getCryptoKeyForJwt(env, kid, alg);
  const params = jwtVerifyParams(alg);
  if (!params) throw new Error(`Unsupported JWT algorithm: ${alg || "unknown"}`);
  const signature = base64UrlToBytes(encodedSig);
  const signedData = new TextEncoder().encode(`${encodedHeader}.${encodedPayload}`);
  const verified = await crypto.subtle.verify(params, key, signature, signedData);
  if (!verified) throw new Error("JWT signature invalid");
  const sub = verifyJwtClaims(payload, env);
  return { payload, sub };
}

async function authenticateRequest(request, env) {
  const sharedKey = String(env.LEDGERAI_SHARED_KEY || "").trim();
  const suppliedSharedKey = String(request.headers.get("x-ledgerai-key") || "").trim();
  if (sharedKey && suppliedSharedKey === sharedKey) {
    return { ok: true, mode: "shared-key", sub: "shared-key" };
  }
  ...
  if (/^Bearer\s+/i.test(authHeader)) {
    ...
    try {
      const verified = await verifySupabaseBearerToken(token, env);
      return { ok: true, mode: "supabase-jwt", sub: verified.sub, payload: verified.payload };
    } catch {
...
// ============================================================================
// KV RETRY QUEUE HELPERS
// Functions for managing the Cloudflare KV-based retry queue
// ============================================================================
...
// Calling OpenAI and Anthropic APIs with retry logic
// ============================================================================

/**
 * Call AI provider (OpenAI or Anthropic) with automatic retry
 * @param {Object} env - Worker environment
 * @param {Object} payload - Request payload with model, max_tokens, messages
 * @returns {Object} { text, usage } on success
 */
...
// ============================================================================
// FX RATE FUNCTIONS
// Currency exchange rate lookup via Frankfurter API
// ============================================================================

/**
 * Fetch foreign exchange rate from Frankfurter API
 * @param {string} from - Source currency code
 * @param {string} to - Target currency code
 * @param {string} date - Date for historical rate (YYYY-MM-DD)
 * @returns {Object} { from, to, requestedDate, rateDate, rate, provider }
 */
...
// Enqueue, process, and pull jobs from the KV-based retry queue
// ============================================================================

/**
 * Enqueue a retry job for background AI processing
 */
...
async function pullCompletedJobs(env, clientId, limit = 40) {
  ...
  while (jobs.length < cap) {
    ...
    for (const k of page.keys) {
      ...
      // Return both completed ("done") and permanently failed jobs
...
export default {
  ...
  async fetch(request, env) {
    ...
    const configError = workerConfigError(env);

    if (request.method === "OPTIONS") {
      return new Response(null, { status: 204, headers: cors });
    }
    if (configError) {
      return json({ error: configError }, 500, cors);
    }
    if (origin && !resolveAllowedOrigin(origin, env.ALLOWED_ORIGIN || "")) {
      return json({ error: "Origin not allowed" }, 403, cors);
    }
    const auth = await authenticateRequest(request, env);
    if (!auth.ok) {
      return json({ error: auth.error || "Unauthorized" }, auth.status || 401, cors);
    }
   ...
   * Processes the retry queue with a larger budget than real-time requests
   */
