...
@app.get("/api/debug/statements")
async def debug_list_statements():
    """Temporary debug: list all statements."""
    stmts = await db.statements.find(
        {}, {"_id": 0, "statement_id": 1, "file_name": 1, "status": 1,
             "account_id": 1, "period_from": 1, "period_to": 1,
             "reconciliation.summary": 1}
    ).to_list(100)
    return stmts


@app.post("/api/debug/reconcile/{statement_id}")
async def debug_reconcile_statement(statement_id: str):
    """Temporary debug endpoint for testing reconciliation without auth.
    ...
    # Also get pending_review transactions to show what's available
    ...
    results["debug"] = {
        ...
        "total_approved_ledger_txns": len(ledger_txns),
        "total_pending_review_txns": len(pending_txns),
        "period": f"{period_from} to {period_to}",
        "account_id": account_id,
        "statement_status": stmt.get("status"),
        "sample_parsed": [{"date": e.get("date"), "amount": e.get("amount"), "desc": e.get("description", "")[:50]} for e in parsed[:3]],
        "sample_ledger": [{"date": t.get("date"), "amount": t.get("amount"), "desc": t.get("description", "")[:50], "status": t.get("status")} for t in ledger_txns[:3]],
        "sample_pending": [{"date": t.get("date"), "amount": t.get("amount"), "desc": t.get("description", "")[:50]} for t in pending_txns[:3]],
    }

    return results


@app.post("/api/debug/approve-all/{statement_id}")
async def debug_approve_all_pending(statement_id: str):
    """Temporary: approve all pending_review transactions for this statement's account+period."""
    stmt = await db.statements.find_one(
        {"statement_id": statement_id}, {"_id": 0}
    )
    if not stmt:
    ...
    parsed = stmt.get("parsed_entries", [])
    dates = [e["date"] for e in parsed if e.get("date")]
    period_from = stmt.get("period_from") or min(dates)
    period_to = stmt.get("period_to") or max(dates)
    account_id = stmt["account_id"]
    user_id = stmt["user_id"]
